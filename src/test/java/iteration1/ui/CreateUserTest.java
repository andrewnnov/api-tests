...
public class CreateUserTest extends BaseUiTest {
    ...
    public void adminCannotCreateUserWithInvalidDataTest() {
        ...
        CreateUserRequestModel newUser = CreateModelSteps.createUserModel();
        newUser.setUsername("a");


        //step 2: admin create user in bank
        new AdminPanelPage().open().createUser(newUser.getUsername(), newUser.getPassword())
                .checkAlertMessageAndAccept(BankAlert.USERNAME_MUST_BE_BETWEEN_3_AND_15_CHARACTERS.getMessage())
                .getAllUsers().findBy(Condition.exactText(newUser.getUsername() + "\nUSER"))
                .shouldNotBe(Condition.exist);

        long usersWithSameUsername = AdminSteps.getAllUsers()
                .stream().filter(user -> user.getUsername()
